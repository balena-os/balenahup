...
def safeCopy(src, dst, buffer_size=1024*1024):
    if os.path.isfile(src):
        return safeFileCopy(src, dst, buffer_size)
    elif os.path.isdir(src):
        return safeDirCopy(src, dst, buffer_size)
    else:
        log.error("Unknown src target to copy " + src + ".")
        return False

def safeDirCopy(src, dst, buffer_size=1024*1024):
    # src must be a dir
    ...
    for root, dirs, files in os.walk(src):
            for name in files:
                ...
                if not safeFileCopy(srcfullpath, dstfullpath, buffer_size):
                    return False
    return True

def safeFileCopy(src, dst, buffer_size=1024*1024):
    # src must be a file
    if not os.path.isfile(src):
        log.error("Can't copy source as " + src + " is not a file.")
        return False

    # Make sure dst is either non-existent or a file (which we overwrite)
    if os.path.exists(dst):
        if os.path.isfile(dst):
            log.warning("Destination file " + dst + " already exists. Will overwrite.")
        elif os.path.isdir(dst):
            log.error("Destination target " + dst + " is a directory.")
            return False
        else:
            log.error("Destination target " + dst + " is unknown.")
            return False

    # Copy file to src.tmp
    srcfd = open(src, 'rb')
    srctmpfd = open(src + ".tmp", "wb")
    while 1:
        buffer = srcfd.read(buffer_size)
        if not buffer:
            break
        else:
            srctmpfd.write(buffer)
    os.fsync(srctmpfd)
    srcfd.close()
    srctmpfd.close()
    ...
    if not os.path.isdir(os.path.dirname(dst)):
        try:
            ...
            log.error("Failed to create directories structure for destination " + dst + ".")
            return False
