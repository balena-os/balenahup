...
if [ -z "${aufs_removed_version}" ]; then
    log ERROR "AUFS removed version not specified."
fi
if version_gt "${target_version}" "${aufs_removed_version}" ||
      [ "$target_version" = "$aufs_removed_version" ]; then
    # Verify 'balena info' succeeds before piping to grep. Collect error output
    # separately to avoid side effect of logging confusing warnings on success as well.
    info_errfile=$(mktemp)
    if info_out=$(balena info 2>"${info_errfile}"); then
        rm -f "${info_errfile}"
        driver=$(grep "Storage Driver:" <<<"${info_out}" || true)
        if [ -z "${driver}" ]; then
            log ERROR "Could not determine storage driver from 'balena info'."
        elif [[ "${driver}" == *aufs* ]]; then
            log ERROR "This device is using the AUFS storage driver and must be migrated before upgrading to version ${aufs_removed_version} or later. \
Upgrade to a release between ${aufs_minimum_migration} and < ${aufs_removed_version}."
        fi
    else
        info_err=$(cat "${info_errfile}")
        rm -f "${info_errfile}"
        log ERROR "Error on command 'balena info': ${info_err}"
    fi
fi
