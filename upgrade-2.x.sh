...
function get_image_location() {
    ...
    image=$(CURL_CA_BUNDLE="${TMPCRT}" ${CURL} \
        -H "Content-Type: application/json" \
        -H "Authorization: Bearer ${APIKEY}" \
        "${API_ENDPOINT}/v6/release?\$select=id&\$expand=contains__image(\$expand=image(\$select=is_stored_at__image_location,content_hash;\$expand=is_a_build_of__service(\$select=service_name)))&\$filter=(belongs_to__application/any(a:a/is_for__device_type/any(dt:dt/slug%20eq%20'${SLUG}')%20and%20is_host%20eq%20true))%20and%20is_invalidated%20eq%20false%20and%20raw_version%20eq%20'${version}'" \
        | jq -r "[.d[] | ((.contains__image | map(.image[0]) | map(select(.is_a_build_of__service[0].service_name == \"hostapp\"))[0]) // .contains__image[0].image[0]) | \"\(.is_stored_at__image_location)@\(.content_hash)\"]")
